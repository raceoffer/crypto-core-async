...
import { Signer as CoreSigner } from 'crypto-core/lib/primitives/ecdsa/signer';

import { wrap, unwrap } from 'crypto-core/lib/marshal';

export class SignerEcdsa {
  constructor(state, worker) {
    this.state = state || { type: 'SignerEcdsa' };
  ...
  static async fromOptions(options, worker) {
    ...
    return worker ? new SignerEcdsa(state, worker) : state;
  }
  ...
  static async fromJSON(json, worker) {
    const state = await SignerEcdsa.invokeStatic({
      method: 'fromJSON',
      arguments: [json]
    }, worker, true);
